...
"""Audit trail for the MCP endpoint — one row per call, read or write.
...
class MCPAccessLog(Document):
	pass


def delete_old_logs():
	"""Daily. Drop rows past the retention window set in MCP Settings."""

	from synapse.mcp_tools.settings import retention_days

	cutoff = frappe.utils.add_days(frappe.utils.nowdate(), -retention_days())
	frappe.db.delete("MCP Access Log", {"creation": ("<", cutoff)})
	frappe.db.commit()
